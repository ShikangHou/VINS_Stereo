    ...
    while (1)
    {
        ...
        if (STEREO)
        {
            ...
            if (!img0.empty())
            {
                ...
                for (auto &f : fi)
                {
                    cv::circle(img0, cv::Point2f(f.second[0].second[3], f.second[0].second[4]), 2, cv::Scalar(0, 255, 0), -1);
                    if (f.second.size() > 1)
                    {
                        cv::circle(img1, cv::Point2f(f.second[1].second[3], f.second[1].second[4]), 2, cv::Scalar(0, 255, 0), -1);
                    }
                }
